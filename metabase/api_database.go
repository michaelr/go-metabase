...
/*
DatabaseApiService Get metadata for a Database
Fetch one Database metadata
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param dbId
@return DatabaseMetadata
*/
func (a *DatabaseApiService) GetDatabaseMetadata(ctx context.Context, dbId int64) (DatabaseMetadata, *http.Response, error) {
	var (
		localVarHttpMethod   = strings.ToUpper("Get")
		localVarPostBody     interface{}
		localVarFormFileName string
		localVarFileName     string
		localVarFileBytes    []byte
		localVarReturnValue  DatabaseMetadata
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/api/database/{dbId}/metadata"
	localVarPath = strings.Replace(localVarPath, "{"+"dbId"+"}", fmt.Sprintf("%v", dbId), -1)
